...
def return_rms_payload(image, lang, num_results, debug=False):
    ...
    logger.log_text("New request")
    logger.log_text("logged_image: " + file_name)
    logger.log_text("text_annotation: " + str(text_ann))
    
    if annotations.best_guess_labels:
        for label in annotations.best_guess_labels:
            output["best_guess"].append(label.label)

    logger.log_text("text_annotation: " + str(output["best_guess"]))
    ...
    for page in annotations.pages_with_matching_images:
        ...
        logger.log_text(page.url)
        analyse_page().start(page, annotations)
    
    print("\n=====\nNumber of threads:")
    print(len(threads_list))
    logger.log_text("thread_used: " + str(len(threads_list)))
    print("Waiting...")
    ...
    if (len(output["full_matches"])+len(output["partial_matches"])+len(output["similar_images"])) < 1:
        ...
        logger.log_text("error: no results")
    else:
        output["results"] = True
